...
export function SendConfirmScreen({ navigation, route }: RootStackScreenProps<'SendConfirm'>) {
  ...
  return (
    <ScreenContainer>
      ...
      <StepHeader step={4} of={4} title="Ton cadeau est prêt ✨" sub="Vérifie & envoie" onBack={() => navigation.goBack()} />
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 22, paddingTop: 20, paddingBottom: 130 }} keyboardShouldPersistTaps="handled">
        ...
        <Pressable onPress={() => navigation.navigate('Customize', { categoryKey, recipientPhone, recipientName, amount })} style={styles.customize}>
          <Text style={styles.customizeText}>✎ Personnaliser la carte</Text>
        </Pressable>

        {/* Delivery channel */}
        <View style={{ marginTop: 14 }}>
          <Text style={styles.label}>Comment le destinataire reçoit la carte ?</Text>
          <SegmentedControl
            value={channel}
            onChange={setChannel}
            options={[
              { value: 'whatsapp', label: '💬 WhatsApp', activeColor: '#25D366' },
          ...
          {channel === 'email' ? (
            <TextInput
              ...
              placeholder="Email du destinataire (ex: kofi@gmail.com)"
            ...
            <TextInput
              ...
              placeholder="Numéro WhatsApp (par défaut = celui du destinataire)"
              placeholderTextColor={colors.ink3}
        ...
        <Card pad={14} style={{ marginTop: 16 }}>
          {[
            { l: 'Destinataire', v: recipientPhone || '—' },
            { l: 'Occasion', v: occasionLabel.replace(/[,!]$/, '') },
          ...
          <View style={[styles.recapRow, { paddingTop: 12 }]}>
            <View>
              ...
              <View style={styles.totalHint}>
                ...
                <Text style={styles.totalHintText}>Aucun frais côté envoi</Text>
              </View>
            </View>
        ...
        {/* Payment method */}
        <Text style={[styles.label, { marginTop: 14 }]}>Avec quoi tu paies ?</Text>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.paymentTitle}>Mon solde Donia</Text>
            <Text style={styles.paymentSub}>
              {walletEnough
                ? `${balance.toLocaleString('fr-FR').replace(/,/g, ' ')} FCFA disponibles`
                : `Solde insuffisant (${balance.toLocaleString('fr-FR').replace(/,/g, ' ')} FCFA)`}
            </Text>
          </View>
          {payMethod === 'wallet' && (
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.paymentSub}>MTN · Moov · Wave · Orange</Text>
          </View>
        ...
        {!walletEnough && (
          <Pressable onPress={() => navigation.navigate('TopUpMethod')} style={styles.topupHint}>
            <Text style={styles.topupHintText}>
              💡 Ou <Text style={{ fontFamily: fonts.bodyBold, color: colors.coral }}>recharge ton solde</Text> d'abord
            </Text>
          </Pressable>
        )}
      </ScrollView>
