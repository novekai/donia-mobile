// Referral — hero indigo "1% à vie" + 2 stats + code + 3 boutons partage
...
const SHARES = [
  { l: 'WhatsApp', bg: '#25D366', emoji: '💬' },
  { l: 'SMS', bg: colors.indigo, emoji: '💌' },
  { l: 'Lien', bg: colors.coral, emoji: '🔗' },
];

function ShareBtn({ s, delay }: { s: typeof SHARES[number]; delay: number }) {
  const bobStyle = useBob({ delay: delay * 1000 });
  return (
    <Animated.View style={[bobStyle, { flex: 1 }]}>
      <Pressable style={[styles.share, { backgroundColor: s.bg }]}>
    ...
    <ScreenContainer>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 22, paddingTop: 20, paddingBottom: 32 }}>
        <BrandGradient variant="indigo" style={[styles.hero, shadow.indigo]}>
          ...
          <HibiscusBurst size={40} color={colors.pink} anim="float" style={{ position: 'absolute', bottom: 10, right: 10, opacity: 0.5 }} />

          <View style={{ alignItems: 'center' }}>
            <Animated.Text style={[bobStyle, { fontSize: 38 }]}>🎉</Animated.Text>
            <Text style={styles.heroTitle}>1% à vie</Text>
        ...
        <View style={styles.statsRow}>
          <Card pad={14} style={styles.stat}>
            <Text style={[styles.statValue, { color: colors.coral }]}>7</Text>
            <Text style={styles.statLabel}>Filleuls actifs</Text>
          </Card>
          <Card pad={14} style={styles.stat}>
            <Text style={[styles.statValue, { color: colors.mango }]}>30 000</Text>
        ...
        <View style={styles.codeBox}>
          <Text style={styles.code}>AWA-2026</Text>
        ...
        <View style={styles.sharesRow}>
          {SHARES.map((s, i) => <ShareBtn key={s.l} s={s} delay={i * 0.3} />)}
        </View>
      </ScrollView>
    </ScreenContainer>
  );
...
const styles = StyleSheet.create({
  ...
  heroTitle: { marginTop: 6, fontFamily: fonts.displayMedium, fontSize: 44, color: colors.mango, letterSpacing: -1.3, lineHeight: 46 },
  ...
  codeBox: { flexDirection: 'row', alignItems: 'center', height: 56, backgroundColor: colors.surface, borderRadius: 16, borderWidth: 1.5, borderStyle: 'dashed', borderColor: colors.coral, paddingHorizontal: 16, gap: 8 },
